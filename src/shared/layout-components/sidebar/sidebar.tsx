...
const Sidebar = ({ local_varaiable, ThemeChanger }: any) => {
	...
	const router = useRouter(); // for router.push('/somewhere')
	const pathname = usePathname(); // for reading the current path
	...
	useEffect(() => {
		...
		if (currentPath !== previousUrl) {
			setMenuUsingUrl(currentPath || "/dashboard/ecommerce");
	...
	function handleAttributeChange(mutationsList: MutationRecord[]) {
		for (const mutation of mutationsList) {
			if (
				...
				let currentPath = window.location.pathname.replace(/\/$/, "") || "/dashboard/ecommerce";
